...
- Native requests are forwarded unchanged apart from model mapping, as in cc-switch, and Copilot validates them. The gateway checks only:
  - The fields it routes on: duplicate top-level members, `model`, `stream`, Responses `previous_response_id`, and the Anthropic version and beta headers.
  - Gateway reasoning handles, which must resolve for the request's account and model.
  - On Messages, top-level reasoning or tool-ownership fields such as `previous_response_id`, `signature` or `tool_use_id`, which are rejected.
- Chat streams always report usage, so an invalid `stream_options` value is replaced with `{"include_usage":true}`.
...
- Record the rule ID of a gateway request rejection. For upstream HTTP errors, they record only an identifier-shaped `error.type` and `error.code`, such as `invalid_request_error`, read from at most 8 KiB of the error body.
