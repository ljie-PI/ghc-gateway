...
test("global header and footer are removed from the direct workspace", async ({ page }) => {
...
test("desktop sidebar stays anchored during page scrolling", async ({ page }, testInfo) => {
  ...
  await expect(sidebar.getByText(/live|reconnecting|connecting/u)).toBeInViewport();
...
test("short desktop and mobile navigation keep all actions reachable", async ({ page }) => {
  ...
  await page.goto("/");
  const sidebar = page.getByRole("complementary", { name: "Primary navigation" });
  const streamState = sidebar.getByText(/live|reconnecting|connecting/u);
  await streamState.scrollIntoViewIfNeeded();
  await expect(streamState).toBeInViewport();
